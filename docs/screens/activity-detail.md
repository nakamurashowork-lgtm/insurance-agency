...
| 項目 | カラム | 必須 | 備考 |
|------|--------|------|------|
| 顧客なしチェック | — | — | チェック時は customer_id を NULL 保存 |
| 顧客 | customer_id | 条件付必須 | 検索可能ドロップダウン。顧客なしチェック時は非活性 |
| 開始時刻 | start_time | 任意 | ポップアップ選択式（時・分） |
| 終了時刻 | end_time | 任意 | ポップアップ選択式（時・分） |
...
#### 画面非表示項目（DDL は維持・hidden input で既存値を保持）

| 項目 | カラム | 非表示理由 |
|------|--------|-----------|
| 活動日 | activity_date | 登録時に確定済み。詳細画面では変更不要 |
| 面談者 | interviewee_name | 使用頻度が低い |
| 結果区分 | result_type | 使用頻度が低い |
| 担当者 | staff_id | 登録時に確定済み。詳細画面では変更不要 |
| 件名 | subject | 内容要約と役割が重複。内容要約で代替可能 |
| 詳細内容 | detail_text | 内容要約と二重。500文字あれば十分 |
| 次回アクション | next_action_note | 内容要約に書けば済む |
| 見込案件 | sales_case_id | 紐づけは見込案件側で行う。活動詳細からの導線は責務違反 |
| 関連満期案件ID | renewal_case_id | IDテキスト入力が破綻している。プルダウン化するまで非表示 |
| 関連事故案件ID | accident_case_id | 同上 |
...
2-3 の入力項目（表示項目のみ）と同一のフィールドを表示・編集可能。画面非表示項目のカラムはそのまま保持される（既存値を上書きしないよう、UPDATE 時も送信しない）。
