...
これをシステムの `common.users.id` に変換しなければ、取り込んだ契約・満期案件に担当者を自動設定できない。

本仕様は、`m_sjnet_staff_mapping` テーブルを介して代理店コードをユーザーIDに解決し、
取込時に `t_contract.sales_user_id` および `t_renewal_case.assigned_user_id` を自動設定するフローを定義する。
...
【STEP 2】m_sjnet_staff_mapping で sjnet_agency_code = 代理店コード を検索する
  → is_active = 1 のレコードが1件ヒット
      → resolved_staff_user_id = user_id
      → staff_mapping_status = 'resolved'

  → ヒット0件（マッピング未登録）
      → resolved_staff_user_id = NULL
...
【STEP 3】解決結果を t_sjnet_import_row に記録する
  ...
  resolved_staff_user_id = 解決したuser_id（未解決はNULL）
...
マッピング解決後の `resolved_staff_user_id` を、契約および満期案件の担当者フィールドに設定する。

```
【t_contract への反映】
  新規INSERT時:
    sales_user_id = resolved_staff_user_id（NULLの場合はNULLのまま）

  既存UPDATE時:
    sales_user_id が現在NULLの場合のみ resolved_staff_user_id で上書きする
    sales_user_id が既に設定済みの場合は上書きしない（手動設定を優先する）

【t_renewal_case への反映】
  新規INSERT時:
    assigned_user_id = resolved_staff_user_id（NULLの場合はNULLのまま）
    office_user_id   = t_contract.office_user_id を引き継ぐ（SJNETには事務担当情報がないため）

  既存UPDATE時:
    assigned_user_id が現在NULLの場合のみ resolved_staff_user_id で上書きする
    assigned_user_id が既に設定済みの場合は上書きしない（手動設定を優先する）
```
...
```
「N件の代理店コードがマッピング未登録です。テナント設定 > SJNETコード設定 で登録してください。」
  → テナント設定へのリンクを付与する
```
...
| 影響箇所 | 具体的な影響 |
|----------|--------------|
| t_contract.sales_user_id | NULL のまま登録される |
| t_renewal_case.assigned_user_id | NULL のまま登録される |
| 満期案件一覧の担当者フィルタ | 「担当者未設定」として表示される |
| 担当者別集計 | その案件が集計から漏れる |

マッピング未設定の状態で運用を開始すると、満期対応の漏れが生じるリスクがある。
初回SJNET取込前に、全代理店コードのマッピング登録を完了させることを強く推奨する。

---

## 7. シードデータ例（実Excel選択リストからの移行）

実際の選択リストシートに記録されていた代理店コードと担当者の対応を、
初期マッピングデータとして登録する際の参考。

```sql
INSERT INTO m_sjnet_staff_mapping
  (sjnet_agency_code, sjnet_staff_name, user_id, is_active, note, created_by, updated_by)
VALUES
  ('N8559000', '飯田',         [user_id_for_iida],        1, NULL,           1, 1),
  ('N8559020', '横手',         [user_id_for_yokote],      1, NULL,           1, 1),
  ('N8559007', '森',           [user_id_for_mori],        1, NULL,           1, 1),
  ('N8559010', '飯田（ササキ）', [user_id_for_iida_sasaki], 1, 'ササキ経由案件', 1, 1),
  ('N8559023', '中村',         [user_id_for_nakamura],    1, NULL,           1, 1),
  ('N8559024', '中村（三浦組）', [user_id_for_nakamura],   1, '三浦組団体担当', 1, 1),
  ('N8559025', '中村眞',       [user_id_for_nakamura_m],  1, NULL,           1, 1),
  ('N8559051', '三浦組団体',    [user_id_for_miura],       1, '法人・団体コード', 1, 1),
  ('N8559008', '増村',         [user_id_for_masumura],    1, NULL,           1, 1);

-- [user_id_for_xxx] の部分は実際のcommon.users.idを設定すること
```
...
行ループ
  ├─ 列44（代理店コード）を取得
  ├─ m_sjnet_staff_mapping で検索 → resolved_staff_user_id を解決
  ├─ 列19（証券番号）で t_contract を検索
  │    → ヒット: UPDATE（sales_user_idが未設定の場合のみ上書き）
  │    → ミス:   INSERT（sales_user_id = resolved_staff_user_id）
  ├─ 列2-3（満期日）で t_renewal_case を検索（contract_id + maturity_date）
  │    → ヒット: UPDATE（assigned_user_idが未設定の場合のみ上書き）
  │    → ミス:   INSERT（assigned_user_id = resolved_staff_user_id）
  └─ t_sjnet_import_row を INSERT
       sjnet_agency_code      = 列44
       sjnet_staff_name       = 列43
       resolved_staff_user_id = 解決結果
       staff_mapping_status   = 'resolved'/'unresolved'/'inactive'/NULL
