...
業務画面のラベル・見出し・補助背景・ボーダーに使用するスレート/ティール系の専用トークンを定義する。
§2-1〜2-3 のセマンティック系トークンとは用途が異なり、構造要素（フォーム、テーブル、コメント、履歴など）の視覚的基盤を担う。

#### テキスト色

| トークン名 | 値 | 主な用途 |
|---|---|---|
| `--text-heading` | `#102a43` | 見出し・サマリーカウント・コメント本文・履歴サマリー |
| `--text-label` | `#334e68` | フォームラベル・フィルターラベル・表示件数ラベル |
| `--text-hint` | `#486581` | kv-list キー・フォームヒント・コメントメタ・履歴メタ |
| `--text-muted-cool` | `#6b7c89` | 完了行テキスト・3次テキスト・パンくずセパレータ |

#### 背景色

| トークン名 | 値 | 主な用途 |
|---|---|---|
| `--bg-subtle` | `#f8fbfc` | コメント・履歴・コンタクトカード・フィルタートグル背景 |
| `--bg-hover-cool` | `#eef6f7` | ホバー状態（モーダル閉じ・ページャー）・履歴テーブルヘッダー |
| `--bg-active-cool` | `#dfeff2` | 現在ページ表示・モバイルメニューアクティブ |

#### ボーダー色

| トークン名 | 値 | 主な用途 |
|---|---|---|
| `--border-cool` | `#d9e2ec` | テーブルボーダー・入力欄ボーダー・ダイアログアクション区切り |
| `--border-cool-accent` | `#a7c1c8` | ページャーホバー・モーダル閉じホバー |
| `--border-cool-muted` | `#bfd8dd` | コンタクトカードボーダー・タグボーダー |

### 2-6. 使用禁止

- 本ドキュメントで定義されたトークン以外のカラー値をナビ・ボタン・バッジ・ボーダー・背景・テキストに直書きしない
- 個別画面で独自カラーを追加しない。追加が必要な場合は本ドキュメントを先に改訂する
- `0.5px` 刻みのフォントサイズを新規に導入しない（§5-2 参照）
...
全画面で共通して参照するスペーシングトークンを定義する。
既存実装の段階的移行を前提とし、新規実装では本トークンの使用を推奨する。

| トークン名 | 値 | 主な用途 |
|---|---|---|
| `--space-xs` | `4px` | バッジ内余白・極小ギャップ |
| `--space-sm` | `8px` | フォーム行間・ボタン間 |
| `--space-md` | `12px` | カード間・セクション間 |
| `--space-base` | `16px` | カード内余白・標準ギャップ |
| `--space-lg` | `20px` | ページコンテナ余白 |
| `--space-xl` | `24px` | 大セクション間 |
| `--space-2xl` | `32px` | ページ下部余白 |
...
「設定」は管理者専用項目であるため、通常ナビ項目と視覚的に分離する。

| 項目 | 値 |
|---|---|
| 実装要素 | `.nav-spacer`（`margin-left: auto` を持つ空要素） |
| 配置 | 「営業活動」と「設定」の間に挿入 |
| 効果 | 「設定」が右端に寄り、一般ナビと分離して表示される |
| 条件 | `admin` / `system_admin` のみ表示される場合に限り挿入する |

実装上、`.nav-spacer { flex: 1; }` または `margin-left: auto` を使用して押し出す。  
スペーサー要素はナビの一部ではなく、レイアウト調整のみを担う。
...
| セクションタイトル | `12px` / `font-weight: 500` / `uppercase` / `letter-spacing: 0.4px` |
...
| 種別 | クラス | 背景 | 文字色 | ボーダー |
|---|---|---|---|---|
| 成功通知 | `.notice` | `--bg-success` | `--text-success` | `0.5px solid --border-success` |
| エラー通知 | `.error` | `--bg-danger` | `--text-danger` | `0.5px solid --border-danger` |
| 警告アラート | `.alert-warn` | `--bg-warning` | `--text-warning` | `0.5px solid --border-warning` |

共通: `padding: 10px 16px` / `border-radius: --radius-md` / `font-size: 13px`

`.alert-error` は `.error` に統合する。新規画面では `.alert-error` を使用しない。

### 6-7. 優先度バッジ

事故案件等の優先度表示にはセマンティックトークンを使用する。

| レベル | 背景 | 文字色 |
|---|---|---|
| 高（high） | `--bg-danger` | `--text-danger` |
| 中（medium） | `--bg-warning` | `--text-warning` |
| 低（low） | `--bg-info` | `--text-info` |
| なし（none） | `--bg-secondary` | `--text-hint` |

共通: `padding: 3px 8px` / `border-radius: 999px` / `font-size: 12px` / `font-weight: 700`

### 6-8. ダイアログ

`.modal-dialog` を全ダイアログの標準クラスとする。

| 項目 | 値 |
|---|---|
| 幅 | `min(640px, calc(100vw - 24px))` |
| 角丸 | `14px` |
| `padding` | `18px 18px 16px` |
| 背景 | `--bg-primary` |
| `box-shadow` | `0 20px 48px rgba(17,33,49,0.24)` |
| `backdrop` | `rgba(16,42,67,0.46)` |

ワイドバリアント: `.modal-dialog-wide`（1200px）、`.modal-dialog-xl`（1100px）

素の `<dialog>` 要素にはリセットスタイルのみ適用し、見た目の定義は `.modal-dialog` クラスに委ねる。
