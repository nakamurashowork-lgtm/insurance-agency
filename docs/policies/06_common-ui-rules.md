...
* タイトル
* 検索条件パネル（デフォルト開く、折りたたみ可）
* 一覧カード（件数・並び順・表示件数・ページャー + テーブル）

### 4-1-1. 検索条件パネルのルール

検索条件は **デフォルトで開いた状態** で表示し、インラインラベル + 最大2段レイアウトで縦スペースを節約する。

**レイアウト**

* ラベルは 12px（インライン表示）
* 入力欄の高さは 30px 統一
* 1段に4〜5フィールドを目安に配置（flex-wrap で自動折り返し）
* 検索・クリアボタンは最終段の右端（`margin-left:auto`）
* スマホ（640px 以下）では縦積みに自動切替

**共通 CSS クラス**（`Layout.php` のインラインスタイルで定義済み）

| クラス | 役割 |
|---|---|
| `search-panel-compact` | パネル本体 |
| `toggle-header` | 折りたたみトグル（クリックで開閉） |
| `search-panel-body` | フォーム本体（`is-collapsed` で非表示） |
| `search-row` | 1段分の横並び |
| `search-field` | ラベル+入力欄のペア |
| `search-label` | インラインラベル |
| `compact-input` | 入力欄本体（`w-date` / `w-sm` / `w-md` / `w-lg`） |
| `search-actions` | 検索・クリアボタン群（右寄せ） |
| `date-range` | 期間フィールド（from〜to、スマホで横並び維持） |

**禁止事項**

* 検索条件をデフォルトで閉じること
* プレースホルダでラベルを代替すること（ラベルは必ず表示）
* 入力欄の高さを画面ごとに変えること
* 4段以上のレイアウト
...
行高のバラつきは縦視線を阻害する。折り返しを禁止し、省略記号で統一する。

**原則**

* 行高は必ず揃える（折り返し禁止）
* 長文列は省略記号（…）で切り、`title` 属性で完全値を hover 表示する
* 短い列（日付・ID・バッジ・金額）は省略不要

**実装**

* テーブルは `table-layout: fixed`（`.table-fixed` クラス）必須
* 省略が必要な `<td>` に `class="cell-ellipsis"` と `title="完全値"` を付与する
* `cell-ellipsis` の CSS は `Layout.php` のインラインスタイルで定義済み
* 日付・金額列は `style="white-space:nowrap;"` を付与して折り返し防止する

**列ごとの扱い**

| 列種別 | 方式 |
|---|---|
| 名前（顧客名・担当者・案件名） | `cell-ellipsis` + `title` |
| 種目・件名・備考 | `cell-ellipsis` + `title` |
| 日付・ID | `white-space:nowrap`（省略なし） |
| ステータスバッジ・優先度 | そのまま表示 |
| 金額・件数 | `white-space:nowrap`（省略なし） |

**禁止事項**

* 省略 CSS を各画面に個別実装すること（`cell-ellipsis` に統一する）
* JavaScript でツールチップを実装すること（`title` 属性で十分）
* `table-layout: auto` の一覧テーブル（省略が効かない）
...
| 成績管理一覧 | `SalesPerformanceListView` |
