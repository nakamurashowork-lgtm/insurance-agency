...
| SCR-CUSTOMER-LIST | 一覧表示 | 検索 | m_customer | なし | ログイン必須、表示範囲制御 |
| SCR-CUSTOMER-DETAIL | 詳細表示 | 取得/更新/保有契約/活動履歴 | m_customer, t_contract, t_activity | なし | ログイン必須、編集可否制御 |
...
必要なDBテーブル: `t_contract`, `t_renewal_case`, `m_customer`, `t_case_comment`, `t_audit_event`, `t_audit_event_detail`
...
必要なDBテーブル: `m_customer`, `t_contract`, `t_activity`
...
### Phase 4B: 成績管理（CSV取込/エラー集約/取込結果表示）
...
位置づけ: 本フェーズは実装完了済みであり、事故業務は満期・顧客・成績と並ぶ標準業務導線として正式に昇格している。
...
- Phase 4A の対象を「成績管理一覧 + 検索 + 登録 + 編集 + 削除」に限定する
...
- 更新
  ...
  - `src/Controller/DashboardController.php`
  - `src/Presentation/DashboardView.php`（「成績管理一覧」カードを実リンク化）
...
- 確認済み: 成績一覧表示、検索、登録反映、編集反映、論理削除反映と一覧非表示、0件検索時表示、他テナント成績の非表示、他テナントID直叩き update / delete の未反映、不正値保存防止、CSRFなし create / update / delete の拒否
- 未実装: CSV取込（Phase 4B）
- 未確認: なし
...
### Phase 4C: 成績管理 CSV取込 成績管理簿対応（追加フェーズ）
...
- テナント分離: tenantA の CSV取込は tenantB 成績に混入しない
...
- `t_notification_delivery` に個別通知成績が記録される
- 同日同対象の再実行は skip 集計になり、重複配信レコードを増やさない
- common/tenant の責務分離が成立（ルート判定: common、対象抽出/成績記録: tenant）
...
## Phase 設定A 受入記録の訂正と追補（2026-04-07）

### 訂正

2026-04-02 の Phase 設定A 受入判定で「目標管理の実装完了」と記録されていたが、
実態調査（2026-04-07）の結果、目標管理機能は未実装であったことが判明した。
当該受入記録は誤りであり、正確には以下の状態であった:

- SJNETコード設定（担当者マスタの sjnet_code 管理）: 実装完了
- 用件区分マスタ: 実装完了
- **目標管理: 未実装（記録と乖離）**

### 追補: 目標管理 最小実装（2026-04-07）

本日、目標管理機能を最小実装として追加した。

実装範囲:
- 年度目標のみ（月次目標は対象外）
- target_type = 'premium_total' のみ
- テナント設定画面内の「目標管理」タブとして追加

新規ファイル:
- src/Domain/Tenant/SalesTargetRepository.php

修正ファイル:
- src/Controller/TenantSettingsController.php（show にデータ取得追加、salesTargetSave / salesTargetDelete ハンドラ追加）
- src/Presentation/TenantSettingsView.php（目標管理タブ・パネル追加）
- src/bootstrap.php（2ルート追加: tenant/sales-target/save, tenant/sales-target/delete）

ドキュメント:
- docs/screens/tenant-settings.md（Section 20 目標管理セクション追加）
- docs/policies/12_sales-target-spec.md（Section 14 実装範囲追加）

未実装（将来拡張）:
- 月次目標の登録・編集
- premium_non_life / premium_life / case_count の登録・編集
- 目標の一括インポート

## insurer_name カラム削除と maturity_date 算出ロジック見直し（2026-04-09）

### 背景

本プロジェクトは損保ジャパン専用のため、保険会社カラム（insurer_name）は業務上情報量ゼロ。また、業務 Excel の運用では保険終期（Q列）を満期日として扱っているが、旧実装は B/C列（満期月/日）から年を動的算出するロジックになっており、業務実態と乖離していた。今回はこれを「削除」ではなく「業務実態への正常化」として対応する。

### 対応内容

- `t_contract.insurer_name` カラムを削除
- `t_sales_performance.insurer_name` カラムを削除
- SJNET 取込で `resolveMaturityDate()` を削除（B/C列から年を算出するロジック）
- SJNET 取込で Q列（保険終期）を `t_renewal_case.maturity_date` にそのまま使う
- SJNET 取込で O列（保険会社）・B列（満期月）・C列（満期日）を不使用カラムに変更
- 全画面から保険会社の表示・入力フォームを削除
- 成績追加ダイアログ（損保・生保とも）から保険会社フィールドを削除
- 仕様書各種（01_canonical-schema.md / sjnet-csv-import-spec.md / renewal-case-list.md / renewal-case-detail.md / sales-performance-list.md / sales-performance-detail.md）を更新

### 残すもの

- SJNET CSV のフォーマットは変更不可（SJNET 側の仕様）
- `t_sjnet_import_row.raw_payload_json` には CSV 全体が JSON で保存されるため、保険会社情報は取込履歴として残る
- 生保会社名は product_type（種目）フィールドに含める運用（例: "損保ジャパンひまわり生命 終身保険"）

### 業務的影響

なし。むしろ業務 Excel の運用と一致し、業務実態に近づく正常化である。

---

## 日報提出機能の UI 削除（2026-04-08）

### 背景
Phase B で実装した日報提出機能（POST activity/submit）は、承認フローが
未実装のため、提出しても業務上何も起きない宙ぶらりんの状態だった。

### 対応
UI から提出機能を完全削除した。
DDL、Repository、Controller、ルートは将来の承認フロー実装に備えて残してある。

### 削除した UI 要素
- 日報ビュー: 提出ボタン、提出状態表示、提出後の読取専用ロジック
- 活動一覧: 日報提出状態フィルタ、「提出済み」バッジ
- ActivityRepository: t_daily_report の JOIN・is_submitted フィルタ

### 残したコード
- t_daily_report.is_submitted, submitted_at カラム
- DailyReportRepository::submit() メソッド
- ActivityController::submit() メソッド（コメント追記済み）
- POST activity/submit ルート

### 復活させる際の手順
1. 承認者側の画面を新規実装
2. 権限制御（提出者・承認者・管理者）を設計
3. 日報ビューの UI を復活（提出ボタン、ステータス表示、読取専用ロジック）
4. 活動一覧のフィルタ・バッジを復活
5. ActivityRepository に t_daily_report JOIN と is_submitted フィルタを復活
6. 仕様書（activity-daily.md, activity-list.md, navigation-policy.md, screen-map.md）を更新
