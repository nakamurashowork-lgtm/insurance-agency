...
{
    public static function render(?string $errorMessage, string $googleLoginUrl, ?array $devLoginSection = null): string
    {
        $errorHtml = '';
        ...
        $content = ''
            ...
            . ($devLoginSection !== null
                ? (static function (array $section): string {
                    $buttonsHtml = '';
                    foreach ($section['buttons'] as $b) {
                        $buttonsHtml .= '<a class="btn btn-secondary" href="' . Layout::escape($b['url']) . '">' . Layout::escape($b['label']) . '</a>';
                    }
                    return '<div class="dev-login-section">'
                        . '<hr style="margin:1.5rem 0;border:none;border-top:1px dashed #ccc;">'
                        . '<p style="font-size:0.75rem;color:#999;margin:0 0 0.5rem;">▼ ' . Layout::escape($section['label']) . '</p>'
                        . '<div style="display:flex;gap:0.5rem;flex-wrap:wrap;">' . $buttonsHtml . '</div>'
                        . '</div>';
                })($devLoginSection)
                : '')
            . '</div>'
