CREATE TABLE IF NOT EXISTS t_renewal_case (
  ...
  case_status         VARCHAR(20)     NOT NULL DEFAULT 'open' COMMENT '案件状態(open/contacted/quoted/waiting/renewed/lost/closed)',
  ...
  CONSTRAINT chk_t_renewal_case_01 CHECK (case_status IN ('open', 'contacted', 'quoted', 'waiting', 'renewed', 'lost', 'closed')),
  CONSTRAINT chk_t_renewal_case_02 CHECK (renewal_result IN ('renewed', 'cancelled', 'lost', 'pending') OR renewal_result IS NULL),
  CONSTRAINT chk_t_renewal_case_03 CHECK (expected_premium_amount IS NULL OR expected_premium_amount >= 0),
  CONSTRAINT chk_t_renewal_case_04 CHECK (actual_premium_amount IS NULL OR actual_premium_amount >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='満期案件';