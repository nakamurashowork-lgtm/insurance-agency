...
config/dml/master/
├── README.md              ← 本ファイル
└── tenant/                ← テナント DB（xs000001_teXXX 等）に投入
...
| 順序 | ファイル | テーブル | 件数 | 用途 |
|---|---|---|---|---|
| 1 | `tenant/01_m_case_status.sql` | `m_case_status` | 13 | 対応状況マスタ（renewal / accident） |
| 2 | `tenant/02_m_sales_case_status.sql` | `m_sales_case_status` | 5 | 見込案件ステータス |
| 3 | `tenant/03_m_activity_type.sql` | `m_activity_type` | 5 | 活動種別（訪問/電話/メール...） |
| 4 | `tenant/04_m_activity_purpose_type.sql` | `m_activity_purpose_type` | 9 | 活動用件区分（満期/新規/事故...） |
| 5 | `tenant/05_m_renewal_method.sql` | `m_renewal_method` | 3 | 更改方法（対面/郵送/電話募集） |
| 6 | `tenant/06_m_procedure_method.sql` | `m_procedure_method` | 7 | 手続方法（対面/対面ナビ...） |
| 7 | `tenant/07_m_product_category.sql` | `m_product_category` | 670+ | SJ-NET 種目マスタ |
| 8 | `tenant/08_seed_internal_customer.sql` | `m_customer` | 1 | 「社内・顧客なし」ダミー顧客 |
| 9 | `tenant/09_m_renewal_reminder_phase.sql` | `m_renewal_reminder_phase` | 2 | 満期通知フェーズ（早期/直前） |
...
```bash
DB=xs000001_te001
for f in config/dml/master/tenant/*.sql; do
  echo "-- applying $f"
...
- 旧 `config/ddl/tenant/seed_internal_customer.sql` → `08_seed_internal_customer.sql`

DDL 側（`config/ddl/tenant/`）には `CREATE TABLE` のみが残る。
